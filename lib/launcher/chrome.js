...
const CRIExtra = require('chrome-remote-interface-extra/lib/chromeRemoteInterfaceExtra')
...
class ChromeLauncher {
  ...
  static async launchB (options) {
    ...
    const theDeathOfChrome = new Promise(resolve => {
    ...
    const killChrome = () => {
      ...
      if (chromeProcess.pid && !chromeProcess.killed && !killed) {
        killed = true
        try {
    ...
    listeners = [
      H.addEventListener(process, 'exit', killChrome),
      H.addEventListener(process, 'SIGINT', () => {
        killChrome()
        process.exit(130)
      }),
      H.addEventListener(process, 'SIGTERM', killChrome),
      H.addEventListener(process, 'SIGHUP', killChrome)
    ]

    try {
      // await Promise.delay(5000)
      const listeningOn = await waitForWSEndpoint(chromeProcess, 30 * 1000)
      const criClient = await CRIExtra(Object.assign({ target: listeningOn }, options))
      const browser = await Browser.create(criClient, {
        process: chromeProcess,
        async closeCallback () {
          H.removeEventListeners(listeners)
          killChrome()
          await theDeathOfChrome
        }
      })
   ...
   * @return {Promise<Browser>}
   */
  static async connectB (options = {}) {
    const { webSocketDebuggerUrl } = await CRIExtra.Version(options)
   ...
   * @desc Launch Chrome by finding an acceptable executable on the host system
   * @param {?ChromeOptions} [options = {}]
   * @return {Promise<{client: CRIConnection, killChrome: function(): void}>}
   */
  static async launch (options) {
    ...
    try {
      ...
      await delay(3000)
      criClient = await CRIExtra(options)
    } catch (e) {
      killChrome()
      throw e
    }
    return { client: criClient, killChrome }
  }
  ...
  static connect (options = {}) {
    return CRIExtra(ensureOptions(options))
  }
  ...
  static async newTab (options = {}) {
    options = ensureOptions(options)
    let target = await CRIExtra.New(options)
    return CRIExtra({ ...options, target })
  }
  ...
  static async getProtocolDef (options = {}) {
    options = ensureOptions(options)
    return CRIExtra.Protocol(options)
  }
}
