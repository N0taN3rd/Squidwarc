...
function warcNamePerURL (outPath) {
  return (seedURL) => path.join(outPath, `${fileNamify(seedURL)}-${moment().format('MM-DD-YYYY_x')}.warc`)
}
...
module.exports = {
  ...
  warcNamePerURL,
  suppliedWarcName,
  apndWarcNamePerURL (config, outPath) {
    let fseed
    return function (seedURL) {
      if (fseed == null) {
        fseed = path.join(outPath, `${fileNamify(seedURL)}-[${config}]-${moment().format('MM-DD-YYYY_x')}.warc`)
      }
      return fseed
    }
  }
