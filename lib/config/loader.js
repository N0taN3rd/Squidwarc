 ...
 */
function badExport (exposed, whatType) {
  console.log(cp.chalk`{bold.red Squidwarc does not know how to handled the supplied function for ${whatType}:
...
`)
  ...
  console.log(cp.chalk.bold.blue('module.exports = async function (pageOrClient) {....}'))
}
...
class Loader {
  ...
  static async load (confPath) {
    ...
    if (chrome.customizer) {
      chrome.customizer = await Loader.loadSomeUserJS(chrome.customizer)
    }
   ...
   */
  static async loadSomeUserJS (jsP, whatType) {
    if (jsP == null) {
      return null
    }
    if (typeof jsP !== 'string') {
      cp.bred(
        `The value for the ${whatType} field found in the supplied config is not a "string" it is a ${typeof jsP}`
      )
      cp.bred(
        `To have Squidwarc use a ${whatType} during crawling please use a string that is a path to the ${whatType}`
      )
      return null
    }
    const scriptPath = path.resolve(untildify(jsP))
    const scriptExists = await fs.pathExists(scriptPath)
    if (!scriptExists) {
      cp.bred(
        `The supplied ${whatType} path does not exist ${jsP} [resolved path = ${scriptPath}]`
      )
      return null
    }
    try {
      ...
      cp.error(`Squidwarc is unable to use the supplied ${whatType} due to an error!`, e)
    }
    return null
  }
  ...
  static async ensureScript (scriptP) {
    const userFN = await Loader.loadSomeUserJS(scriptP, 'user script')
