...
const DEFAULT_ARGS = require('../launcher/defaultArgs')
const RequestMonitor = require('../../node-warc/lib/requestCapturers/puppeteer')
...
class PuppeteerCrawler extends EventEmitter {
  ...
  async init () {
    cp.error('This crawler is still under development use at your own risk', new Error('Entering Dev Mode'))
    ...
    // await this.page.goto('https://www.reuters.com/', {waitUntil: 'networkidle0'})
    ...
    if (!wasError) {
      ...
      try {
        await this._client.send('Page.addScriptToEvaluateOnNewDocument', pageEvals.noNaughtyJs)
      } catch (error) {
        await this._client.send('Page.addScriptToEvaluateOnLoad', pageEvals.noNaughtyJS2)
      }
      this.requestMonitor.attach(this._client)
      this._ua = await this.getUserAgent()
      if (!this._navMan.pageLoadStrat) {
        // this._client.Page.loadEventFired(this._autoScroller)
        await this._client.send('Page.addScriptToEvaluateOnNewDocument', pageEvals.addAutoScroll)
      } else {
   ...
   */
  async getOutLinks () {
    let evaled = await this._client.send('Runtime.evaluate', pageEvals.getLinks)
