...
class PuppeteerCrawler extends EventEmitter {
  ...
  async init () {
    cp.error('This crawler is still under development use at your own risk',
      new Error('Entering Dev Mode'))
    ...
    if (!wasError) {
      ...
      await this._client.send('Page.addScriptToEvaluateOnNewDocument',
        InjectManager.getInitInject())
      await this._client.send('Page.addScriptToEvaluateOnNewDocument',
        InjectManager.getScollerInject())
  ...
  async genWarc ({info, outlinks}) {
    ...
    info.warcInfoDescription = info.warcInfoDescription ||
      this.options.versionInfo.warcInfoDescription
    await this._warcGenerator.writeWarcInfoRecord(info.isPartOfV,
      info.warcInfoDescription, this._ua)
    await this._warcGenerator.writeWarcMetadataOutlinks(this._currentUrl,
      outlinks)
    this.requestMonitor.stopCapturing()
   ...
   */
  async getOutLinks () {
    let evaled = await this._client.send('Runtime.evaluate', InjectManager.getCollectInject())
