...
const shouldQ = {
  [cmodePAL]: function (url, curURL, tracker) {
    const ext = Path.extname(url.pathname)
    if (ext !== '') {
      return !bigExtLookup[ext] && !tracker.seen.has(url.href)
    }
    return !tracker.seen.has(url.href)
  },
  [cmodePO]: function (url, curURL, tracker) {
    const ext = Path.extname(url.pathname)
    if (ext !== '') {
      return !bigExtLookup[ext] && !tracker.seen.has(url.href)
    }
  ...
  [cmodePSD]: function (url, curURL, tracker) {
    const cDomain = parseDomain(curURL)
    const ext = Path.extname(url.pathname)
    const td = parseDomain(url.host)
    const tdTest = td && cDomain.domain === td.domain
    if (ext !== '') {
      return !bigExtLookup[ext] && tdTest && !tracker.seen.has(url.href)
    }
    return tdTest && !tracker.seen.has(url.href)
  }
}
    ...
    if (Array.isArray(seeds)) {
      return seeds.map(aSeed => {
        if (typeof aSeed === 'object') {
          return {
            url: aSeed.url,
            mode: Frontier.crawlModeToSymbol(aSeed.mode || mode),
      ...
      return {
        url: seeds.url,
        mode: Frontier.crawlModeToSymbol(seeds.mode || mode),
    ...
    while (i < len) {
      ...
      if (ext !== '') {
        if (
          td &&
          !bigExtLookup[ext] &&
          !tracker.seen.has(url.href) &&
          cDomain.domain === td.domain
        ) {
          tracker.seen.add(url.href)
          next.push({
            url: url.href,
