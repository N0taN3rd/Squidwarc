...
const RequestMonitor = require(
  '../../node-warc/lib/requestCapturers/remoteChrome')
...
class ChromeCrawler extends EventEmitter {
  ...
  constructor (options = defaultOpts) {
    ...
    this._autoScroller = this._autoScroller.bind(this)
  }
  ...
  _autoScroller () {
    this._client.Runtime.evaluate(pageEvals.makeSmoothScroll(1000), noop)
  }
  ...
  async _pageLoaded (info) {
    if (this.options.scroll) {
      try {
        await this._client.Runtime.evaluate(
          pageEvals.makeSmoothScroll(this.options.scroll)
        )
      } catch (error) {
