...
class Launcher {
          ...
          if (matchAr !== null && matchAr.length > 0) {
            ...
            while (i < len) {
              if (whichOne[execs[i]] !== undefined) {
                maybeString = await exec(`grep -E "^Exec=\/.*\/(google|chrome|chromium)-.*" ${whichOne[execs[i]]} | awk -F '=' '{print $2}'`)
                theExe = Array.from(new Set(maybeString.split(nlre).map(it => it.replace(argumentsRegex, '$1'))))[0]
                if (bingo(theExe)) {
                  return theExe
                }
              }
  ...
  static async launch (options) {
    ...
    chromeArguments.push(`--user-data-dir=${options.userDataDir || fakeUdir}`, `--remote-debugging-port=${options.connect.port}`)
    if (options.args) {
      chromeArguments.push(...options.args.filter(arg => !arg.startsWith('--user-data-dir=')))
    }
    if (options.headless) {
    ...
    const chromeProcess = cp.spawn(chromeExecutable, chromeArguments, {stdio: ['ignore', 'ignore', 'pipe']})
    ...
    try {
      ...
      let listeningOn = await waitForWSEndpoint(chromeProcess, options.timeout || 30 * 1000)
  ...
  static async launchNoConnect (options) {
    ...
    let killed = false
    process.once('exit', killChrome)
    chromeProcess.once('close', () => fs.removeSync(fakeUdir))
